...
module.exports = (req, res) => { 
    ...
    if (req.method === 'GET' && pathname === '/api/goals') {
        ...
        //Zobrazení všech cílů (nebo filtrovaných)
        res.writeHead(200, { 'Content-Type': 'application/json' });
        return res.end(JSON.stringify(goals));
    }

    //Zobrazení detailu jednoho cíle
    ...
    if (pathname === '/api/goals' && method === 'POST') {
        ...
        req.on('end', () => {
            ...
            // VALIDACE
            if (!newGoal.name || newGoal.name.trim() === "") {
                res.writeHead(400, { "Content-Type": "application/json" });
                res.end(JSON.stringify({ error: "Goal name is required." }));
                return;
            }

            if (!newGoal.category || newGoal.category.trim() === "") {
                res.writeHead(400, { "Content-Type": "application/json" });
    ...
    if (pathname === '/api/goals' && method === 'PUT') {
        ...
        req.on('end', () => {
            ...
            // VALIDACE
            if (!updatedGoal.name || updatedGoal.name.trim() === "") {
            res.writeHead(400, { "Content-Type": "application/json" });
            res.end(JSON.stringify({ error: "Goal name cannot be empty." }));
            return;
            }
